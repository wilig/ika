...
symbol_table_t *symbol_table_init(allocator_t allocator, scope_t *scope) {
  ...
  printf("symbol table got hashtbl_address %p\n", table);
  symtbl->table = table;
...
symbol_table_entry_t *symbol_table_lookup(symbol_table_t *symtbl, str key) {
  str_entry_t *entry = hashtbl_str_lookup(symtbl->table, key);
  if (entry) {
    return (symbol_table_entry_t *)entry->value;
  } else if (symtbl->scope->parent) { // Traverse up the chain trying to resolve
                                      // the symbol
    printf("Traversing to parent\n");
    return symbol_table_lookup(symtbl->scope->parent->symbol_table, key);
  }
  return NULL;
}
...
void symbol_table_insert(symbol_table_t *t, str key, e_ika_type type,
  ...
  entry->name =
      &key; // TODO: Make a strcpy and use it to create a copy of the key
  ...
  if (!hashtbl_str_insert(
          t->table, (str_entry_t){.key = key, .valid = true, .value = entry})) {
